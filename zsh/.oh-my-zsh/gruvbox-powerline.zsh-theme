...
[[ -n "$ZSH_POWERLINE_DIRECTORY_DEPTH" ]]       || ZSH_POWERLINE_DIRECTORY_DEPTH=1
...
if [ $ZSH_POWERLINE_SHOW_USER = true ]; then
	local USER="%n"
    PROMPT="${PROMPT}${FG_COLOR_BASE02}${BG_COLOR_BASE3}${PADDING}${USER}"
	PADDING=' '
fi
...
if [ $ZSH_POWERLINE_SHOW_IP = true ]; then
    ...
    IP=`curl -si --max-time 2 http://ipecho.net/plain`
       ...
       if [ $ZSH_POWERLINE_SHOW_USER = true ]; then
               PROMPT="${PROMPT}${FG_COLOR_GREEN}${BG_COLOR_BASE3} @"
       fi
...
if [ $ZSH_POWERLINE_SHOW_OS = true ]; then
	PROMPT="${PROMPT}${FG_COLOR_BASE3}${BG_COLOR_BASE01}${PADDING}${LOGO}"
	PADDING=' '
fi
